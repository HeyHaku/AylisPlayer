...
public class NativeMediaPlayerCore implements
    ...
    public NativeMediaPlayerCore(Context context, String playerName, OnNotifyListener onNotifyListener)
    {
    ...
    void preparePlayer(int thisPlayer, Uri path, float fadeStartVolume, long seekPos) {
        ...
        try {
            ...
            if (scheme == null || "file".equals(scheme)) {
                String filePath = path.getPath() != null ? path.getPath() : path.toString();
                java.io.File file = new java.io.File(filePath);
                java.io.FileInputStream fis = new java.io.FileInputStream(file);
                players[thisPlayer].player.setDataSource(fis.getFD());
                fis.close();
            } else if ("content".equals(scheme)) {
                android.os.ParcelFileDescriptor pfd = context.getContentResolver().openFileDescriptor(path, "r");
                if (pfd != null) {
                    players[thisPlayer].player.setDataSource(pfd.getFileDescriptor());
                    pfd.close();
                } else {
    ...
    public long duration() {
        ...
        return players[currentPlayer].player.getDuration();
    }

    public long position() {
        if (players[currentPlayer].getStateLevels() < PlayerEntry.state_stopped) return 0;
        return players[currentPlayer].player.getCurrentPosition();
    }
