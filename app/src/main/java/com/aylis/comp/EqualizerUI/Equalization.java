...
public class Equalization {
    ...
    public static void getEqBandsBassTrebleControl(EQPreset preset, EQPreset bassPreset, EQPreset treblePreset, float bassValue, float trebleValue, float[] eqBandsNormalOut, float[] eqBandsFreq)
    {
    ...
    static void pointCurveToEqBands(PointCurve pointCurve, float[] eqBandsNormalOut, float[] eqBandsFreq)
    {
        float loLog = (float) Math.log10(loFreq);
        float hiLog = (float) Math.log10(hiFreq);
        float divider = hiLog - loLog;
