...
public class PlaybackDesign {
    ...
    public static volatile boolean wantsPlaying;
    public static volatile PlaylistSong currentDisplayTrack = PlaylistSong.EmptySong;
    public static volatile PlaylistSong.Data songDisplyData = PlaylistSong.emptyData;
    public static volatile PlayingMediaInfo playingMediaInfo = PlayingMediaInfo.empty;
    public static volatile long trackPosition = 0;
    public static volatile int repeatMode = 0;
    public static volatile int volumeMax = 10;
    public static volatile boolean volumeMuted = false;
    public static volatile boolean timeoutEnabled = true;
    ...
    public PlaybackDesign() {
        ...
        if (ctx != null) {
            ...
            if (playbackQueue != null) {
                Tuple2<PlaylistSong, IItemIdentifier> queueEntry = playbackQueue.getCurrentQueueEntry();
                if (queueEntry != null && queueEntry.obj1 != null) {
                    currentDisplayTrack = queueEntry.obj1;
                    songDisplyData = currentDisplayTrack.getDataBlocking(ctx);
                    
                    android.content.SharedPreferences mPreferences = AppPreferences.createOrGetInstance().getPreferences(ctx);
                    long seekPos = mPreferences.getLong("seekpos", 0);
                    trackPosition = seekPos;
                    
                    // We don't have duration info easily available here without MediaPlaybackService,
                    // but we can pass duration from songDisplyData if available, or just leave it empty.
                    playingMediaInfo = new PlayingMediaInfo(songDisplyData.duration > 0 ? songDisplyData.duration : -1, false);
                }
            }
        }
        ...
        MediaPlaybackService.onDisplayMetaDataStateChanged.subscribeWeak(new WeakEvent4.Handler<PlaylistSong, IItemIdentifier, PlaylistSong.Data, PlayingMediaInfo>() {
            ...
            public void invoke(PlaylistSong currentTrack, IItemIdentifier currentItemIdent, final PlaylistSong.Data currentTrackData, PlayingMediaInfo playingMediaInfo) {
                ...
                threadHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        com.aylis.comp.LibraryQueueUI.FragmentLibrary.onTrackInfoChanged.invoke(currentTrackData);
        ...
        MediaPlaybackService.onPlaybackCompleted.subscribeWeak(new WeakEvent3.Handler<Integer, Boolean, Long>() {
            ...
            public void invoke(final Integer rptMode, Boolean wntsPlaying, final Long atTime) {
                ...
                if (com.aylis.comp.export.ExportVideoActivity.isExporting) {
                    return;
                }
        ...
        MediaControlsUI.onSetAudioStereoBalance.subscribeWeak(new WeakEvent2.Handler<Integer, Integer>() {
            ...
            public void invoke(Integer val, Integer valMax) {
                AppPreferences.createOrGetInstance().setInt(AppPreferences.PREF_Int_volumeStereoBalance, (val - 5) * 20);
            }
        }, listenerRefHolder);
